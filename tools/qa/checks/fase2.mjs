...
export async function executar({ browser, dir }) {
  ...
  // ---- 4. abas mobile aprovadas -------------------------------------------
  {
    ...
    v.check(
      "4. Abas do mobile levam às telas dos rótulos (Início, Agenda, Escalas, Contribuições, Perfil)",
      problemas.length === 0,
    ...
    const destacada = await sessao.page.evaluate(() => {
      const rotulos = ["Início", "Agenda", "Escalas", "Contribuições", "Perfil"];
    ...
    const esperadaNaRota = { "/home": "Início", "/agenda": "Agenda", "/schedules": "Escalas", "/statement": "Contribuições", "/profile": "Perfil" }[caminho(sessao.page.url())];
    v.check(
